...
{{range  .AmcWiseSection}}
   {{if Contains .IssuerName "TOTAL"}}
    ...
    {{if Contains .IssuerName "GRAND TOTAL"}}{{$TotalLable ="Grand Total"}}{{end}}
...
{{$ReportCount:=0}}
{{if .DebtMfAmcAllocation}}
{{$ReportCount = 1}}
{{end}}
{{if .DebtMfSectorAllocation}}
{{$ReportCount = 1}}
{{end}}
{{if .DebtMfAvgMatAllocation}}
{{$ReportCount = 1}}
{{end}}
{{if .DebtQuants}}
{{$ReportCount = 1}}
{{end}}
{{if .DebtMfRatingAllocation}}
{{$ReportCount = 1}}
{{end}}
{{if .DebtMfInstrumentSecAllocation}}
{{$ReportCount = 1}}
{{end}}
...
#let DbetMfInstrumentData = (
  {{range .DebtMfInstrumentSecAllocation}}
  ( value: {{ConvertToFormattedPercentagePointer .Percentage}},
     name: "{{ .InstrumentName}}"
  ),
{{end}}
...
#let DebtMfAvgMatData = (
  {{range .DebtMfAvgMatAllocation}}
  (value: {{ConvertToFormattedPercentagePointer .Percentage}}, name: "{{.MaturityBucket}}"),
  {{end}}
 )
   ...
   // Header row
      ...
      // Data rows
       ...
       table.cell( text("{{.Metric}}")),
        table.cell(text("{{ConvertToFormattedPercentagePointer .ValueYear}}")),
        {{end}}
)
  ]
  ]
