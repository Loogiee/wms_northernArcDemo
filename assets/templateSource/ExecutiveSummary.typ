...
#pagebreak()
{{range $data := .ExecutiveSummary.BasicInformationSection }}
#text("{{ .Description}}", size: 20pt)
{{end}}
#pagebreak()

#set page(fill: rgb("#f8f8f8"))
...
#place(
  dx: 75%,
  dy: -7%,
  [#image("assets/images/kfintech-logo.png", width: 350pt)],
)
#place(
  dy: 20pt,
  [#line(length: 100%)],
...
#let smallCardList = (
  {{range $data := .ExecutiveSummary.BasicInformationSection }}
  (description: "{{ .Description}}", value: "{{ConvertToFormattedNumberWithoutDecimalPointer .Value}}", date: "{{ .Date}}", Image: "./assets/images/one.png"),
{{end}}
  // (description: "INFLOW MINUS OUTFLOW", Value: "₹ -26,78,06,427.9", Date: "Since Inception", Image: "assets/images/one.png"),
  // (description: "HOLDING COST", Value: "₹ 1,32,48,47,581.1", Date: "as on 30 Jun 2022", Image: "assets/images/two.png"),
  // (description: "CURRENT VALUE", Value: "₹ 1,64,89,89,920.8", Date: "as on 30 Jun 2022", Image: "assets/images/three.png"),
    ...
    ..smallCardList.map(item => {
      box(
        ...
        // Display description for all cards
          ...
          text(item.value, size: 20pt, weight: "extrabold")
        } else {
